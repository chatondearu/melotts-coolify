...
**First `docker compose up --build`**: expect a substantial build with large downloads and sufficient **RAM/disk** on the builder; CPU PyTorch by default keeps this more predictable than an unconstrained `torch` install from PyPI. The [Dockerfile](Dockerfile) patches upstream **`torch.load`** for **PyTorch 2.6+** (`weights_only=False`) and sets **`use_hf=False`** on **`TTS(...)`** in `melo/init_downloads.py`, **`melo/app.py`**, and **`melo/main.py`** so checkpoints come from MyShell **S3** URLs (see upstream `download_utils.py`) instead of Hugging Face—more reliable on CI and Coolify builders.
